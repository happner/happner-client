(function(isBrowser) {
  ...
  HappnerClient.prototype.newDescription = function(description) {
    Object.keys(description.components).forEach(componentName => {
      if (['security', 'system', 'rest', 'api', 'data'].indexOf(componentName) > -1) return;
  ...
  HappnerClient.prototype.newComponent = function(component) {
    this.__apis.forEach(api => {
      if (
        ...
        this.log.info(
          `mounting methods for discovered component: [${component.componentName}], running on member: [${component.member}]`
        );
        this.__mountMethods(api, component.description, component.componentName);
      }
      if (api.event[component.componentName] == null) {
        this.log.info(
          `mounting events for discovered component: [${component.componentName}], running on member: [${component.member}]`
        );
    ...
    for (var i = 0; i < componentNames.length; i++) {
      ...
      if (api.exchange[componentName]) {
        ...
        if (semver.coercedSatisfies(api.exchange[componentName].__version, component.version)) {
          this.log.warn(
            `component version compare failed, component ${componentName} existing version: ${api.exchange[componentName].__version}, incoming version: ${component.version}`
          );
          continue;
        }
      }
  ...
  HappnerClient.prototype.__mountExchange = function(api, componentName, version, methodName) {
    ...
    if (!api.exchange[componentName][methodName]) {
      this.log.info(`mounting method: [${componentName}.${methodName}] with version: ${version}`);
