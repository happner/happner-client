(function(isBrowser) {
  ...
  HappnerClient.prototype.mount = function(orchestrator) {
    ...
    this.on('peer/arrived/description', this.newDescription);
  ...
  HappnerClient.prototype.newDescription = function(component) {
    this.__apis.forEach(api => {
      if (!api.exchange[component.componentName]) return;
      if (api.exchange[component.componentName].__discover) {
        this.log.info(`discovered component [${component.componentName}]`);
