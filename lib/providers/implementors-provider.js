(function(isBrowser) {
    ...
    Object.keys(_this.dependencies).forEach(function(dependorName) {
      Object.keys(_this.dependencies[dependorName]).forEach(function(componentName) {
        ...
        (countMatches === 0
          ? _this.log.warn
          : _this.log
              .info)('dependent %s has %d of %s %s', dependorName, countMatches, componentName, version);

        _this
          .__getUpdatedDependencyDescriptions(descriptions, componentName, version)
          .forEach(foundComponentDescription => {
            _this.happnerClient.emit('peer/arrived/description', {
  ...
  ImplementorsProvider.prototype.__getUpdatedDependencyDescriptions = function(
    ...
    return descriptions.filter(description => {
      if (!description.components || !description.components[componentName]) return false;
      if (semver.satisfies(description.components[componentName].version, version)) return true;
    });
  };

  ImplementorsProvider.prototype.__getUpdatedDependencyDescription = function(
    descriptions,
    componentName,
    version
  ) {
    if (Array.isArray(descriptions)) {
      return descriptions.find(function(description) {
    ...
    if (semver.satisfies(descriptions.components[componentName].version, version))
      return descriptions;
  };
