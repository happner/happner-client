(function(isBrowser) {
  ...
  // var Promise; // /api/client already defined Promise
  var semver;
  ...
  ImplementorsProvider.prototype.addPeer = function(name) {
    ...
    var onSuccess = function(description) {
      ...
      _this.happnerClient.emit('peer/add', clonedDescription);
    };
  ...
  ImplementorsProvider.prototype.removePeer = function(name) {
    ...
    if (removedPeerDescription) this.happnerClient.emit('dependency-peer-removed', removedPeerDescription);
  };
  ...
  ImplementorsProvider.prototype.logDependenciesMet = function(descriptions) {
    ...
    Object.keys(_this.dependencies).forEach(function(dependorName) {
      ...
      Object.keys(_this.dependencies[dependorName]).forEach(function(componentName) {
        ...
        (countMatches == 0 ? _this.log.warn : _this.log.info)(
          'dependent %s has %d of %s %s', dependorName, countMatches, componentName, version
        );

        if (countMatches == 1) {
          var foundComponentDescription = _this.__getUpdatedDependencyDescription(descriptions, componentName, version);
          if (foundComponentDescription) {
            _this.happnerClient.emit('dependency-met', {
  ...
  ImplementorsProvider.prototype.__getUpdatedDependencyDescription = function(descriptions, componentName, version) {
    if (Array.isArray(descriptions)){
      return descriptions
        .find(function (description) {
          if (!description.components || !description.components[componentName]) return;
          if (semver.satisfies(description.components[componentName].version, version))
            return description;
        });
    }
    if (!descriptions.components || !descriptions.components[componentName]) return;
    if (semver.satisfies(descriptions.components[componentName].version, version)) return descriptions;
  };
